    ...
    <div class="card shadow-sm mt-4">
        ...
        <div class="card-body">
            ...
            <div class="col-md-4 d-flex align-items-end">
                ...
                <button id="resetBtn" class="btn btn-secondary text-white px-4 shadow-sm">Reset</button>
            </div>
        </div>
    ...
    $(document).ready(function () {
        ...
        function getRowHtml(data = {}) {
            ...
            return `<tr class="rule-row">
                    ...
                    <td><select class="form-control form-control-sm mode-id"  data-selected="${data.payment_mode_id || ''}"></select></td>
        ...
        // Function for getting the mode based on the service Id
        function loadServiceModes($row) {

            let serviceId = $row.find('.row-service').val();
            let $target = $row.find('.mode-id');
            ...
            const url = "{{route('get_modeId_based_on_serviceId',['id' => ':id'])}}".replace(':id', serviceId);
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    let selectedValue = $target.data('selected');

                    if (res.data && res.data.length) {
                        res.data.forEach(item => {
                            let selected = (selectedValue == item.mode_id) ? 'selected' : '';
                            options += `<option value="${item.mode_id}" ${selected}>
                                ${item.mode_name}
                            </option>`;
                        });
                    }
                ...
                error: function () {
                    $target.html('<option value="">Error loading</option>');
                }
            });
        }

        // Getting the mode on onchange event of the serviceId
        $(document).on('change', '.row-service', function () {
            let $row = $(this).closest('tr');
            loadServiceModes($row);
        });
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.status) {
                        ...
                        // });
                        res.scheme.rules.forEach(rule => {
                            let $row = $(getRowHtml(rule));
                            $('#rulesTable tbody').append($row);
                            loadServiceModes($row);
                        });
                    ...
                    if (res.status) {
                        ...
                        Swal.fire({
                            ...
                            .then(() => {
                                ...
                                schemeTable.ajax.reload(null, false);
                            });
                    ...
                    if (res.status) {
                        ...
                        Swal.fire({
                            ...
                            .then(() => {
                                ...
                                relationTable.ajax.reload(null, false);
                            });
