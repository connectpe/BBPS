...
@section('content')
    ...
    <div class="modal fade" id="serviceProductModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="serviceTitle">Service Modes</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <form id="serviceProductForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex justify-content-center mt-2">
                            <button type="button" class="btn buttonColor btn-sm" onclick="addProductRow()">
                                + Add More Product
                            </button>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#servicesTable').DataTable({
                ...
                onclick="openServiceProducts(${row.id}, '${row.service_name}')">
            ...
            var table = $('#userServicesTable').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: function(row) {
                            const url = "{{ route('view_user', ['id' => 'id']) }}".replace('id', row
                                .user_id);
    ...
    <script>
        ...
        function openServiceProducts(id, name) {
            serviceId = id;
            rowIndex = 0;

            $("#service_id").val(id);
            $("#serviceTitle").text("Service Modes - " + name);
            $("#productBody").html("");

            $.ajax({
                url: `/admin/payment-modes/${id}`,
                type: "GET",
                success: function(response) {
                    ...
                    $("#serviceProductModal").modal("show");
                }
            });
        }

        function appendProductRow(product = null) {

            let html = `
        <tr>
            <input type="hidden" name="products[${rowIndex}][id]" value="${product?.id ?? ''}">

            <td>
                <input class="form-control"
                    name="products[${rowIndex}][mode_name]"
                    value="${product?.mode_name ?? ''}"
                    placeholder="Enter Mode">
            </td>

            <td>
                <input class="form-control"
                    name="products[${rowIndex}][mode_slug]"
                    value="${product?.mode_slug ?? ''}"
                    placeholder="Enter Slug">
            </td>

            <td>
                <input class="form-control"
                    name="products[${rowIndex}][min_order_value]"
                    type="number"
            ...
            <td>
                <input class="form-control"
                    name="products[${rowIndex}][max_order_value]"
                    type="number"
                    value="${product?.max_order_value ?? ''}">
            </td>

            <td>
                <input class="form-control"
                    name="products[${rowIndex}][tax_value]"
                    type="number"
    ...
    <div class="form-check form-switch d-flex justify-content-center">
        <input class="form-check-input"
            type="checkbox"
            ${product ? (product.is_active == '1' ? 'checked' : '') : 'checked'}
            onchange="toggleStatus(this, ${product?.id ?? 0})"
        ...
        $('#serviceProductForm').on('submit', function(e) {
            e.preventDefault();
            $.ajax({
                url: `/admin/payment-modes/save/${serviceId}`,
                type: "POST",
                ...
                success: function(response) {
                    ...
                    $('#serviceProductModal').modal('hide');
                },
                error: function(xhr) {
                    let message = 'Something went wrong';
        ...
        function toggleStatus(toggleElement, productId) {
            ...
            let statusText = toggleElement.checked ? 'activate' : 'deactivate';

            Swal.fire({
                title: `Are you sure?`,
                text: `Do you want to ${statusText} this mode?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes',
                cancelButtonText: 'No'
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `/admin/payment-modes/status/${productId}`,
                        type: "POST",
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        ...
                        success: function(response) {
                            toggleElement.checked = response.is_active == '1';
